...
const REPO_OVERRIDABLE = new Set(['ignore', 'extra_ignore', 'guidelines_files'])
...
function loadConfig() {
  ...
  for (const raw of readFileSync(path, 'utf8').split('\n')) {
    ...
    if (!REPO_OVERRIDABLE.has(key)) {
      console.warn(
        `[warn] ${path}: "${key}" is owned centrally by acid-info/ai-review and was ignored. ` +
  ...
  // Must happen before the caller snapshots cfg.ignore into `ignoreRes`.
